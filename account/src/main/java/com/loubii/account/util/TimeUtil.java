...
//    }
    ...
    public static Date getFirstDayOfWeek(int year, int week) {
        ...
        // 强制设置为周一开始（对应你原有的 2 - dayofweek 逻辑）
        cal.set(Calendar.DAY_OF_WEEK, Calendar.MONDAY);
        return getDayStartTime(cal.getTime());
    }

    /**
     * 【新增】通过年份和一年中的第几周获取该周最后一天
     */
    public static Date getEndDayOfWeek(int year, int week) {
        Calendar cal = Calendar.getInstance();
        cal.setTime(getFirstDayOfWeek(year, week));
        cal.add(Calendar.DAY_OF_WEEK, 6); // 加6天到周日
        return getDayEndTime(cal.getTime());
    }

    /**
     * 【新增】获取指定年份、指定月份的第一天
     * Calendar月份从0开始，所以 month 需要减 1
     */
    public static Date getFirstDayOfMonth(int year, int month) {
        Calendar cDay = Calendar.getInstance();
        cDay.set(Calendar.YEAR, year);
        cDay.set(Calendar.MONTH, month); // 注意：此处根据你原有 getFirstDayOfMonth(int month) 风格保持一致
        cDay.set(Calendar.HOUR_OF_DAY, 0);
        cDay.set(Calendar.MINUTE, 0);
